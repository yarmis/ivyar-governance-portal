...
import { CheckCircle2, AlertTriangle, XCircle, FileText, Download, Share2, ExternalLink } from 'lucide-react';
...
export default function AdvancedResultPanel({ decision, scenario, input }: Props) {
  ...
  const statusConfig = {
    approve: {
      ...
      text: 'text-green-400',
      label: t.hbsAutopilot?.results?.approved || 'APPROVED'
    },
    review: {
      color: 'yellow',
      icon: AlertTriangle,
      ...
      label: t.hbsAutopilot?.results?.reviewRequired || 'REVIEW REQUIRED'
    },
    reject: {
      color: 'red',
      icon: XCircle,
      ...
      text: 'text-red-400',
      label: t.hbsAutopilot?.results?.rejected || 'REJECTED'
    }
  };
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom">
      ...
      <div className={`bg-bg-surface border ${config.border} rounded-card p-8`}>
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-heading-l mb-1">{t.hbsAutopilot?.results?.decisionResult || 'Decision Result'}</h2>
              <p className="text-body-m text-text-secondary">
                {t.hbsAutopilot?.results?.evaluated || 'Evaluated'}: {scenario.name}
              </p>
            </div>
          </div>
          ...
          <div className="relative w-48 h-48">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-label-m text-text-secondary mt-1">
                {t.hbsAutopilot?.results?.confidence || 'Confidence'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-8">
          <div className="text-center p-4 bg-bg-obsidian rounded-lg">
            ...
            <div className="text-label-s text-text-secondary">{t.hbsAutopilot?.results?.highConfidence || 'High Confidence'}</div>
          </div>
          <div className="text-center p-4 bg-bg-obsidian rounded-lg">
            <div className="text-heading-m text-yellow-400 mb-1">50-79</div>
            <div className="text-label-s text-text-secondary">{t.hbsAutopilot?.results?.mediumConfidence || 'Medium Confidence'}</div>
          </div>
          <div className="text-center p-4 bg-bg-obsidian rounded-lg">
            <div className="text-heading-m text-red-400 mb-1">0-49</div>
            <div className="text-label-s text-text-secondary">{t.hbsAutopilot?.results?.lowConfidence || 'Low Confidence'}</div>
          </div>
        </div>
      </div>
      {/* Explanation Card */}
      <div className="bg-bg-surface border border-border-subtle rounded-card p-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-heading-m">{t.hbsAutopilot?.results?.aiReasoning || 'AI Reasoning'}</h3>
        </div>
      ...
      {decision.references && decision.references.length > 0 && (
        <div className="bg-bg-surface border border-border-subtle rounded-card p-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-heading-m">{t.hbsAutopilot?.results?.references || 'References & Citations'}</h3>
          </div>
      ...
      {/* Input Summary */}
      <div className="bg-bg-surface border border-border-subtle rounded-card p-8">
        <h3 className="text-heading-m mb-4">{t.hbsAutopilot?.results?.inputSummary || 'Input Data Summary'}</h3>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          {t.hbsAutopilot?.results?.exportDecision || 'Export Decision'}
        </button>
        ...
        >
          ...
          {t.hbsAutopilot?.results?.viewRawJSON || 'View Raw JSON'}
        </button>
        ...
        >
          ...
          {t.hbsAutopilot?.results?.shareDecision || 'Share Decision'}
        </button>
      </div>
