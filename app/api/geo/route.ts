...
type FloodZoneType = 'A' | 'AE' | 'AH' | 'AO' | 'V' | 'VE' | 'X' | 'X500' | 'D';
type RiskLevel = 'minimal' | 'low' | 'moderate' | 'high' | 'severe';
...
const UA_UTILITIES = [
  { id: 'ua-water-001', countryCode: 'UA', name: 'Kyivvodokanal', shortName: 'KVK', utilityType: 'water', oblast: 'Kyiv', customers: 2900000, bbox: { minLng: 30.2, minLat: 50.3, maxLng: 30.8, maxLat: 50.6 }, contact: { phone: '+380 44 238 3838', emergency: '104' } },
  { id: 'ua-elec-001', countryCode: 'UA', name: 'DTEK Kyiv Electric', shortName: 'DTEK', utilityType: 'electricity', oblast: 'Kyiv', customers: 1200000, bbox: { minLng: 30.2, minLat: 50.3, maxLng: 30.8, maxLat: 50.6 }, contact: { phone: '+380 44 467 3456', emergency: '103' } },
  { id: 'ua-gas-001', countryCode: 'UA', name: 'Kyivgaz', shortName: 'KG', utilityType: 'gas', oblast: 'Kyiv', customers: 850000, bbox: { minLng: 30.2, minLat: 50.3, maxLng: 30.8, maxLat: 50.6 }, contact: { phone: '+380 44 238 0404', emergency: '104' } },
  { id: 'ua-heat-001', countryCode: 'UA', name: 'Kyivteploenergo', shortName: 'KTE', utilityType: 'heat', oblast: 'Kyiv', customers: 12000, bbox: { minLng: 30.2, minLat: 50.3, maxLng: 30.8, maxLat: 50.6 }, contact: { phone: '+380 44 247 3838', emergency: '15-88' } },
  { id: 'ua-water-002', countryCode: 'UA', name: 'Kharkivvodokanal', shortName: 'KhVK', utilityType: 'water', oblast: 'Kharkiv', customers: 1400000, bbox: { minLng: 36.1, minLat: 49.9, maxLng: 36.4, maxLat: 50.1 }, contact: { phone: '+380 57 733 2222', emergency: '104' } },
];

const US_UTILITIES = [
  { id: 'us-elec-001', countryCode: 'US', name: 'Pacific Gas and Electric', shortName: 'PG&E', utilityType: 'electricity', state: 'CA', customers: 16000000, bbox: { minLng: -122.5, minLat: 37.5, maxLng: -121.5, maxLat: 38.5 }, contact: { phone: '1-800-743-5000', emergency: '1-800-743-5002' } },
  { id: 'us-elec-002', countryCode: 'US', name: 'Consolidated Edison', shortName: 'ConEd', utilityType: 'electricity', state: 'NY', customers: 10000000, bbox: { minLng: -74.1, minLat: 40.5, maxLng: -73.7, maxLat: 41.0 }, contact: { phone: '1-800-752-6633', emergency: '1-800-752-6633' } },
  { id: 'us-elec-003', countryCode: 'US', name: 'LADWP', shortName: 'LADWP', utilityType: 'electricity', state: 'CA', customers: 4000000, bbox: { minLng: -118.7, minLat: 33.7, maxLng: -118.1, maxLat: 34.4 }, contact: { phone: '1-800-342-5397' } },
  { id: 'us-elec-004', countryCode: 'US', name: 'Duke Energy', shortName: 'Duke', utilityType: 'electricity', state: 'NC', customers: 7800000, bbox: { minLng: -81.5, minLat: 34.5, maxLng: -79.5, maxLat: 36.5 }, contact: { phone: '1-800-777-9898' } },
  { id: 'us-water-001', countryCode: 'US', name: 'Miami-Dade Water and Sewer', shortName: 'MDWASD', utilityType: 'water', state: 'FL', customers: 2300000, bbox: { minLng: -80.5, minLat: 25.5, maxLng: -80.1, maxLat: 26.0 }, contact: { phone: '305-665-7477' } },
];

const TECHNICAL_CONDITIONS = [
...
function isPointInBbox(coords: Coordinates, bbox: { minLng: number; minLat: number; maxLng: number; maxLat: number }): boolean {
...
function assessFloodRisk(zones: typeof FLOOD_ZONES) {
  const sorted = zones.sort((a, b) => {
    const riskOrder: Record<RiskLevel, number> = { severe: 4, high: 3, moderate: 2, low: 1, minimal: 0 };
    return riskOrder[b.riskLevel as RiskLevel] - riskOrder[a.riskLevel as RiskLevel];
  });
...
export async function GET(request: NextRequest) {
  ...
  try {
    switch (action) {
      // ==================== DASHBOARD ====================
      case 'dashboard': {
        const allUtils = [...UA_UTILITIES, ...US_UTILITIES];
        const byType: Record<string, number> = {};
        allUtils.forEach(u => { byType[u.utilityType] = (byType[u.utilityType] || 0) + 1; });
        ...
        return NextResponse.json({
          ...
          data: {
            utilities: {
              total: allUtils.length,
              ua: UA_UTILITIES.length,
              us: US_UTILITIES.length,
              totalCustomers: allUtils.reduce((s, u) => s + u.customers, 0),
              byType,
            },
            serviceAreas: {
            ...
            coverage: {
              uaOblasts: [...new Set(UA_UTILITIES.map(u => u.oblast))].length,
              usStates: [...new Set(US_UTILITIES.map(u => u.state))].length,
            },
          },
        });
      }
      ...
      case 'utilities': {
        ...
        let utilities = country === 'UA' ? UA_UTILITIES : US_UTILITIES;
        if (type && type !== 'all') utilities = utilities.filter(u => u.utilityType === type);
      ...
      case 'utilities-all': {
        return NextResponse.json({ 
          success: true, 
          data: { ua: UA_UTILITIES, us: US_UTILITIES },
          total: UA_UTILITIES.length + US_UTILITIES.length,
        });
      }

      case 'utility': {
        const id = searchParams.get('id');
        const utility = [...UA_UTILITIES, ...US_UTILITIES].find(u => u.id === id);
      ...
      case 'utilities-within': {
        ...
        const allUtils = country === 'UA' ? UA_UTILITIES : US_UTILITIES;
        const found = allUtils.filter(u => isPointInBbox(coords, u.bbox));
      ...
      case 'location-lookup': {
        ...
        const utilities = (detectedCountry === 'UA' ? UA_UTILITIES : US_UTILITIES)
          .filter(u => isPointInBbox(coords, u.bbox));
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'report-issue': {
        const { type, coordinates, description, contactEmail } = body;
